"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('@angular/core');
var web_storage_service_1 = require('./util/web-storage.service');
var ToDoService = (function () {
    function ToDoService(_webStorgeService) {
        this._webStorgeService = _webStorgeService;
        this.collectionName = "to-do-list";
    }
    ToDoService.prototype.save = function (toDo) {
        return this._webStorgeService.save(this.collectionName, toDo);
    };
    ToDoService.prototype.delete = function (toDo) {
        return this._webStorgeService.delete(this.collectionName, toDo);
    };
    ToDoService.prototype.list = function () {
        return this._webStorgeService.list(this.collectionName);
    };
    ToDoService.prototype.get = function (id) {
        return this._webStorgeService.get(this.collectionName, id);
    };
    ToDoService = __decorate([
        core_1.Injectable(), 
        __metadata('design:paramtypes', [web_storage_service_1.WebStorgeService])
    ], ToDoService);
    return ToDoService;
}());
exports.ToDoService = ToDoService;
//# sourceMappingURL=to-do.service.js.map